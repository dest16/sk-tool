...
一个单容器、自托管的简体中文 Web 应用：搜索 Sukebei Nyaa、将选中的磁力链接交给内置 Transmission 下载，并在完成后整理到固定目录。
...
| 容器路径 | 用途 |
| --- | --- |
| `/config` | SQLite、Transmission 会话和初始化状态 |
| `/downloads` | Transmission 的下载根目录；每个任务直接写入文件或 torrent 内容目录，不再嵌套任务 UUID |
...
| `51413/tcp`、`51413/udp` | Transmission BitTorrent/DHT 入站端口 |

Transmission 已启用 DHT、PEX、LPD、固定 peer 端口和 UPnP 端口转发。Compose 使用 host 网络，让 Transmission 直接看到宿主机局域网接口并向路由器申请 51413/TCP+UDP 映射；因此端口不再使用 Docker `ports` 转发。若端口被占用，可在 `.env` 中修改 `ARIA2_P2P_PORT`，若 Web 端口被占用则修改 `WEB_PORT`，然后重新创建容器。RPC 端口仍只监听容器/宿主机回环地址，不对外暴露。
...
下载任务的“删除任务”会停止 Transmission 任务，并在确认框中默认勾选删除该任务在 `/downloads` 下的文件；删除成功后任务记录也会从下载列表和数据库中移除。已整理入库的 `/library` 文件不会被删除。已完成的任务可使用“删除任务文件”清理下载目录中的残留并删除任务记录；“删除历史”只删除记录。取消勾选则只删除任务记录、保留下载文件；如果文件系统拒绝删除，任务会保留并显示错误，便于处理权限后重试。
...
- 只有管理员会话可以搜索、创建和控制任务。
- Transmission RPC 只监听容器回环地址。
- 只接受合法 BTIH magnet；整理操作拒绝路径穿越、符号链接和目标覆盖。
- 站点和下载代理可在设置中分别配置，凭据不会出现在 API 响应或日志中。
