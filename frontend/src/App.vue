...
const statusLabels: Record<string, string> = {
  ...
  moving: "整理中", moved: "已整理", conflict: "名称冲突", failed: "失败", cancelled: "已取消",
};
const activeDownloads = computed(() => downloads.value.filter((item) => !["moved", "failed", "cancelled"].includes(item.status)));
  ...
  try {
    ...
    if (!setupRequired.value) {
      try {
        ...
        await loadMeta(); await refreshDownloads(); await loadProxy();
...
async function doSetup() {
  ...
  try { const data = await api("/api/setup", { method: "POST", body: JSON.stringify({ setup_token: setup.token, username: setup.username, password: setup.password }) });
    loggedIn.value = true; setupRequired.value = false; username.value = data.username; csrf.value = data.csrf_token; await loadMeta(); notice.value = "初始化成功";
  } catch (err) { error.value = (err as Error).message; }
}
async function doLogin() {
  error.value = "";
  try { const data = await api("/api/auth/login", { method: "POST", body: JSON.stringify(login) }); loggedIn.value = true; username.value = data.username; csrf.value = data.csrf_token; await loadMeta(); await refreshDownloads(); await loadProxy(); }
  catch (err) { error.value = (err as Error).message; }
}
...
async function taskAction(task: Download, action: string) {
  if (["cancel", "move", "cleanup"].includes(action) && !window.confirm(action === "cancel" ? "取消后会保留暂存文件，确定继续吗？" : action === "cleanup" ? "确认删除该任务的暂存文件吗？此操作不可恢复。" : "确认将文件移动到整理目录吗？")) return;
  try { const data = await api(`/api/downloads/${task.id}/${action}`, { method: "POST", body: "{}" }); const index = downloads.value.findIndex((item) => item.id === task.id); if (index >= 0 && data.task) downloads.value[index] = data.task; await refreshDownloads(); }
  catch (err) { error.value = (err as Error).message; }
}
