...
const proxy = reactive({ indexer_proxy: "", downloader_proxy: "" });
const proxyConfigured = reactive({ indexer_proxy: false, downloader_proxy: false });
...
async function loadProxy() { const data = await api("/api/settings/proxy"); proxy.indexer_proxy = data.indexer_proxy || ""; proxy.downloader_proxy = data.downloader_proxy || ""; proxyConfigured.indexer_proxy = data.indexer_proxy_configured; proxyConfigured.downloader_proxy = data.downloader_proxy_configured; }
...
async function saveProxy() { try { const update: Record<string, string> = {}; if (proxy.indexer_proxy) update.indexer_proxy = proxy.indexer_proxy; if (proxy.downloader_proxy) update.downloader_proxy = proxy.downloader_proxy; const data = await api("/api/settings/proxy", { method: "PUT", body: JSON.stringify(update) }); proxyConfigured.indexer_proxy = data.indexer_proxy_configured; proxyConfigured.downloader_proxy = data.downloader_proxy_configured; proxy.indexer_proxy = ""; proxy.downloader_proxy = ""; notice.value = "代理设置已保存（已配置的值不会回显）"; } catch (err) { error.value = (err as Error).message; } }
